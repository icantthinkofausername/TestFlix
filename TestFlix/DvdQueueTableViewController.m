...
@interface DvdQueueTableViewController () <RKRequestDelegate, RKObjectLoaderDelegate>

@end

@implementation DvdQueueTableViewController
...
-(void) loadQueue
{
    if([[self oauthViewControllerTouch] checkAuthorizationForOperation:@selector(loadQueue) forOperationController: self withNavController: [self navigationController]]) {
        ...
        [queueStrUrl appendString:@"/queues/disc"];
